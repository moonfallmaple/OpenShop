...
export class Counter extends Component {
  render() {
    const {countAtShop,colorAtShop,increment,changeColor}=this.props
    return (
      <div className="App">
        <span style={{color:colorAtShop}} >{countAtShop}</span>
        <button onClick={increment}  style={{color:colorAtShop}} >+1</button>
        <button onClick={changeColor} style={{color:colorAtShop}} >blue</button>
      </div>
    );
  }
}
...
export const changeColor={type:'CHANGE_COLOR', colorInStore:"blue"}
